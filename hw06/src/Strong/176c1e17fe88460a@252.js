...
function _chart_transition(data,selectedSeries,d3)
{
  // 定義圖形 
...
function _chart_mouse(data,selectedSeries,d3)
{
  // 定義圖形 
  const margin = {top: 20, right: 30, bottom: 30, left: 40};
  const width = 500 - margin.left - margin.right;
  ...
  series.forEach((serie) => {
      ...
      bars.enter().append("rect")
          ...
          .on("mouseout", function(_evt, d) {
              d3.select(this).attr("fill", colorScale(d3.select(this).datum().data.series));
          })
